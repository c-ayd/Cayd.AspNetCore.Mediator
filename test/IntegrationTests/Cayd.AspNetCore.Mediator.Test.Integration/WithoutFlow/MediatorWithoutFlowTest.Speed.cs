...
namespace Cayd.AspNetCore.Mediator.Test.Integration.WithoutFlow
{
    public partial class MediatorWithoutFlowTest
    {
        ...
        public async Task SpeedEndpoint_WhenRequestsAreMade_ShouldTakeLongerOnceAndThenFast()
        {
            ...
            var coldStartResponse = await _testHostWithoutFlowFixture.Client.GetAsync(_initializeEndpoint);
            if (!coldStartResponse.IsSuccessStatusCode)
                Assert.Fail("The cold start went wrong. Rerun the test.");

            // Act
            startTime = DateTime.UtcNow;
            var initialRequest = await _testHostWithoutFlowFixture.Client.GetAsync(_speedEndpointMediatorOn);
            ...
            {
                startTime = DateTime.UtcNow;
                var response = await _testHostWithoutFlowFixture.Client.GetAsync(_speedEndpointMediatorOn);
                endTime = DateTime.UtcNow;
            ...
            {
                startTime = DateTime.UtcNow;
                var response = await _testHostWithoutFlowFixture.Client.GetAsync(_speedEndpointMediatorOff);
                endTime = DateTime.UtcNow;
